# Todo App Intermediate Level Constitution
...
- Data model: Task as dataclass with id (int, auto-increment), title (str), description (str), completed (bool, default False), priority (str, default "medium"), tags (list[str], default empty list)
...
- Command set (case-insensitive, trimmed whitespace):
  ...
  - Intermediate Commands:
    ...
    - list --overdue     → show only overdue tasks (when advanced features added)
- Display: Enhanced table with columns ID, Status ([x]/[ ]), Priority, Title, Tags, Description (truncated)
...
- /src/
  ...
  ├── models.py           # Task dataclass (enhanced with priority and tags)
  ├── todo_manager.py     # Core CRUD and future business logic (enhanced with search/filter/sort)
  ├── cli.py              # Command parsing, display, and user interaction (enhanced with flag support)
...
- Intermediate features (priorities, tags, search, filter, sort) fully functional
- Enhanced table display includes Priority and Tags columns with proper formatting
- Command filtering and sorting work as specified (list --priority high, list --sort priority, etc.)
...
### Intermediate Level (Current Extension – Organization & Usability)
...
This constitution serves as the foundational governing document for Phase II development; All development activities must comply with these principles; Amendments require documentation and approval; Code reviews must verify adherence to all principles; Version: 2.0.0 | Ratified: 2025-01-28 | Last Amended: 2025-01-28